...
| **Student talk ratio** | Student speaking time ÷ total session time. VAD on each participant stream (or diarization if mixed); sum speaking seconds per participant.                                             | Yes — running ratio; nudge if tutor talk >80% or student silent >N min               | Yes — total ratio; breakdown by session segment                              |
